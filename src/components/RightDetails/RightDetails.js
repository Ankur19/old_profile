...
const heading = "Relevant Coursework";
...
function RightDetails(props){
    ...
    let unorderedList = [];
    for(let i=0;i<courseWork[props.curEdu].length;i++){
        unorderedList.push(<li key={i}>{courseWork[props.curEdu][i]}</li>);
    }
    return <div id="rightdetails-main-div">
        <div className="rightdetails-heading">{heading}</div>
